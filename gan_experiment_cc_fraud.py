...
illicit_count = creditcard_labels[creditcard_labels == 1].shape[0]
...
scaler = MinMaxScaler()
scaler.fit(creditcard_dataset[creditcard_labels == 1])
iter_dataset = scaler.transform(creditcard_dataset)
# batch_count = (creditcard_dataset.shape[0] - creditcard_dataset[creditcard_labels == 1].shape[0]) // illicit_count
batch_count = 100
for batch in range(batch_count):
    ...
    for i in range(training_steps):
        ...
        random_min = 0
        random_max = 1
        random_noise = np.random.uniform(low=random_min, high=random_max, size=(illicit_count, iter_dataset.shape[1])) * torch.rand((illicit_count, iter_dataset.shape[1])).numpy()
        generated_data = generator(torch.tensor(random_noise).float())
        true_data, true_labels = iter_dataset[creditcard_labels == 1], np.ones(iter_dataset[creditcard_labels == 1].shape[0])
        ...
        generator_discriminator_loss = loss(generator_discriminator_out, torch.zeros(illicit_count).unsqueeze(1))
...
print(f"ACTUAL    : {[np.percentile(creditcard_dataset[creditcard_labels == 1], x).round(decimals=2) for x in percentiles]}")
print(f"GENERATED : {[np.percentile(generated_illicit_transactions, x).round(decimals=2) for x in percentiles]}")

creditcard_dataset = np.insert(creditcard_dataset, LABEL_INDEX, np.zeros(creditcard_dataset.shape[0]), axis=1)
...
full_dataset = np.concatenate((creditcard_dataset, generated_illicit_transactions),axis=0)
full_labels = np.concatenate((creditcard_labels, np.ones(generated_illicit_transactions.shape[0])),axis=0)
...
X_train, X_test, y_train, y_test = train_test_split(creditcard_df.drop("label",axis=1), creditcard_df["label"], train_size=0.7, shuffle=True)
clf = RandomForestClassifier(n_estimators=100)
...
print(f"f1: {f1_score(y_test, pred)}")
print(f"roc_auc  : {roc_auc_score(y_test, pred)}")
print(f"average_precision_score  : {average_precision_score(y_test, pred)}")
print(f"accuracy  : {accuracy_score(y_test, pred)}")
